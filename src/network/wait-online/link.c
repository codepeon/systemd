...
int link_update_monitor(Link *l) {
        ...
        r = network_link_get_operational_state(l->ifindex, l->manager->namespace, &l->operational_state);
        if (r < 0)
                ret = log_link_debug_errno(l, r, "Failed to get operational state, ignoring: %m");

        r = sd_network_link_get_required_family_for_online(l->ifindex, l->manager->namespace, &required_family);
        if (r < 0)
        ...
        r = sd_network_link_get_ipv4_address_state(l->ifindex, l->manager->namespace, &ipv4_address_state);
        if (r < 0)
        ...
        r = sd_network_link_get_ipv6_address_state(l->ifindex, l->manager->namespace, &ipv6_address_state);
        if (r < 0)
